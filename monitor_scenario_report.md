...
### 1. 프로그램 로그 데이터 분석 — 타임스탬프 기반 실행 순서 패턴화

아래 로그는 Healthy System Monitoring 시나리오에서 `agent-leak-app-x86`의 내부 스케줄러가 Thread-A, Thread-B, Thread-C를 처리하는 전 과정을 기록한 것이다.

**원본 실행 로그**

```
2026-06-27 16:09:21,420 [INFO] [Scheduler] Task Scheduler Initialized.
2026-06-27 16:09:21,420 [INFO] [Scheduler] Registered Tasks: ['Thread-A', 'Thread-B', 'Thread-C']
2026-06-27 16:09:21,420 [INFO] [Scheduler] Starting task execution...
2026-06-27 16:09:21,420 [INFO] [Thread-A] Task Started. Calculating... (20%)
2026-06-27 16:09:21,472 [INFO] [Thread-A] Calculating... (40%)
2026-06-27 16:09:21,523 [INFO] [Thread-A] Preempted. Progress saved at (40%)
2026-06-27 16:09:21,575 [INFO] [Thread-B] Task Started. Calculating... (20%)
2026-06-27 16:09:21,627 [INFO] [Thread-B] Calculating... (40%)
2026-06-27 16:09:21,679 [INFO] [Thread-B] Preempted. Progress saved at (40%)
2026-06-27 16:09:21,731 [INFO] [Thread-C] Task Started. Calculating... (20%)
2026-06-27 16:09:21,783 [INFO] [Thread-C] Calculating... (40%)
2026-06-27 16:09:21,835 [INFO] [Thread-C] Preempted. Progress saved at (40%)
2026-06-27 16:09:21,887 [INFO] [Thread-A] Resumed. Calculating... (60%)
2026-06-27 16:09:21,939 [INFO] [Thread-A] Calculating... (80%)
2026-06-27 16:09:21,991 [INFO] [Thread-A] Preempted. Progress saved at (80%)
2026-06-27 16:09:22,043 [INFO] [Thread-B] Resumed. Calculating... (60%)
2026-06-27 16:09:22,095 [INFO] [Thread-B] Calculating... (80%)
2026-06-27 16:09:22,147 [INFO] [Thread-B] Preempted. Progress saved at (80%)
2026-06-27 16:09:22,199 [INFO] [Thread-C] Resumed. Calculating... (60%)
2026-06-27 16:09:22,250 [INFO] [Thread-C] Calculating... (80%)
2026-06-27 16:09:22,302 [INFO] [Thread-C] Preempted. Progress saved at (80%)
2026-06-27 16:09:22,354 [INFO] [Thread-A] Resumed. Calculating... (100%)
2026-06-27 16:09:22,405 [INFO] [Thread-B] Resumed. Calculating... (100%)
2026-06-27 16:09:22,457 [INFO] [Thread-C] Resumed. Calculating... (100%)
2026-06-27 16:09:22,508 [INFO] [Scheduler] All tasks completed.
```
...
| 타임스탬프 | 스레드 | 진행률 | 이벤트 | 이전 대비 간격 |
|-----------|--------|--------|--------|--------------|
| 16:09:21,420 | Thread-A | 20% | 시작 | — |
| 16:09:21,472 | Thread-A | 40% | 계산 중 | +52ms |
| 16:09:21,523 | Thread-A | — | **선점 (40% 저장)** | +51ms |
| 16:09:21,575 | Thread-B | 20% | 시작 | +52ms |
| 16:09:21,627 | Thread-B | 40% | 계산 중 | +52ms |
| 16:09:21,679 | Thread-B | — | **선점 (40% 저장)** | +52ms |
| 16:09:21,731 | Thread-C | 20% | 시작 | +52ms |
| 16:09:21,783 | Thread-C | 40% | 계산 중 | +52ms |
| 16:09:21,835 | Thread-C | — | **선점 (40% 저장)** | +52ms |
| 16:09:21,887 | Thread-A | 60% | 재개 | +52ms |
| 16:09:21,939 | Thread-A | 80% | 계산 중 | +52ms |
| 16:09:21,991 | Thread-A | — | **선점 (80% 저장)** | +52ms |
| 16:09:22,043 | Thread-B | 60% | 재개 | +52ms |
| 16:09:22,095 | Thread-B | 80% | 계산 중 | +52ms |
| 16:09:22,147 | Thread-B | — | **선점 (80% 저장)** | +52ms |
| 16:09:22,199 | Thread-C | 60% | 재개 | +52ms |
| 16:09:22,250 | Thread-C | 80% | 계산 중 | +51ms |
| 16:09:22,302 | Thread-C | — | **선점 (80% 저장)** | +52ms |
| 16:09:22,354 | Thread-A | 100% | 재개 → 완료 | +52ms |
| 16:09:22,405 | Thread-B | 100% | 재개 → 완료 | +51ms |
| 16:09:22,457 | Thread-C | 100% | 재개 → 완료 | +52ms |
| 16:09:22,508 | — | — | 전체 완료 | +51ms |

**도출된 수치 요약**

| 항목 | 값 |
|------|-----|
| 로그 이벤트 간격 (스텝) | 약 **52ms** (일정) |
| 스레드당 1회 할당 시간 (타임 퀀텀) | 약 **104ms** (2스텝 × 52ms) |
| 전체 실행 시간 | **1,088ms** (21,420 → 22,508) |
| 실행 순서 | A → B → C → A → B → C → A, B, C (완료) |
| 순환 주기 (3개 스레드 1라운드) | 약 **312ms** (104ms × 3) |

**실행 사이클 구조**

```
[ Cycle 1 ] 21,420 ~ 21,835 (약 415ms)
  A(20→40%) → B(20→40%) → C(20→40%)

[ Cycle 2 ] 21,887 ~ 22,302 (약 415ms)
  A(60→80%) → B(60→80%) → C(60→80%)

[ Cycle 3 ] 22,354 ~ 22,457 (약 103ms)
  A(100%) → B(100%) → C(100%)  ← 잔여 작업 처리 후 전체 완료
```

---

### 2. 알고리즘 역추론 — Round-Robin 판정

#### 결론

> **현재 프로그램에 적용된 스케줄링 알고리즘은 Round-Robin(RR)이다.**

#### 판정 근거

| 근거 | 로그 내 증거 |
|------|------------|
| **고정 타임 퀀텀** | 모든 스레드가 정확히 104ms(2스텝) 실행 후 선점됨 |
| **강제 선점** | `Preempted. Progress saved at (X%)` — 작업 완료 전 강제 중단 |
| **컨텍스트 저장 및 복원** | 선점 시 진행률 저장, 재개 시 저장 지점에서 정확히 이어서 시작 (`Resumed. Calculating... (X%)`) |
| **엄격한 순환 순서** | A → B → C → A → B → C, 예외 없이 동일 순서 반복 |
| **균등 CPU 할당** | 세 스레드 모두 매 라운드 104ms씩 동일하게 배분 |

#### FCFS·Priority가 아닌 이유

**FCFS (First Come, First Served)**

FCFS는 비선점 방식으로, 먼저 도착한 태스크가 완료될 때까지 CPU를 독점한다.
로그에서 Thread-A는 20% 시점에 선점되어 Thread-B에 CPU를 넘겨주므로 FCFS와 상충한다.
FCFS였다면 실행 순서는 `A(0→100%) 완료 → B(0→100%) 완료 → C(0→100%) 완료`여야 한다.

**Priority (우선순위 스케줄링)**

우선순위 기반이라면 스레드별 우선순위 차이가 로그에 나타나야 하며, 높은 우선순위 스레드가 더 많은 CPU 시간을 할당받아야 한다.
세 스레드는 완전히 동일한 할당 시간(104ms)과 동일한 실행 순서를 유지하며, 우선순위 차등의 흔적이 없다.

---

### 3. Round-Robin 스케줄링의 장단점 및 적합 아키텍처

#### 기술적 장점

| 장점 | 설명 |
|------|------|
| **공정성(Fairness)** | 모든 태스크에 동일한 CPU 시간을 순서대로 배분 — 특정 태스크가 CPU를 독점하지 않음 |
| **기아 현상(Starvation) 없음** | 순환 구조이므로 모든 태스크가 반드시 실행 기회를 얻음 |
| **응답 시간 예측 가능** | 타임 퀀텀과 대기 태스크 수만 알면 최대 응답 대기 시간 산출 가능 |
| **선점 기반 반응성** | 특정 태스크가 오래 걸려도 다른 태스크가 중단 없이 진행됨 |

#### 기술적 단점

| 단점 | 설명 |
|------|------|
| **컨텍스트 스위칭 오버헤드** | 선점마다 현재 상태를 저장하고 다음 태스크 상태를 복원하는 비용 발생 |
| **타임 퀀텀 크기에 민감** | 퀀텀이 너무 작으면 스위칭 오버헤드 과다, 너무 크면 사실상 FCFS와 동일해짐 |
| **처리량(Throughput) 비최적** | 짧은 태스크와 긴 태스크를 동일하게 처리하므로 짧은 태스크의 완료가 지연될 수 있음 |
| **캐시 지역성 저하** | 빈번한 컨텍스트 스위칭이 CPU 캐시를 무효화하여 캐시 미스 증가 |

#### 서비스 유형별 적합성

**적합한 서비스 — 실시간 응답이 중요한 웹/API 서버**

다수의 클라이언트 요청을 동시에 처리해야 하는 웹 서버나 REST API 서버는 RR에 적합하다.
각 요청이 공정하게 CPU 시간을 배분받으므로 특정 요청이 응답 불가 상태(기아)에 빠지지 않으며, 응답 지연 시간(Latency)이 예측 가능한 범위 안에서 관리된다.
사용자 체감 응답성이 중요한 인터랙티브 서비스(채팅 서버, 실시간 대시보드, 게임 세션 서버 등)에도 유효하다.

**덜 적합한 서비스 — 처리량이 중요한 배치 서버**

대용량 파일 변환, 데이터 집계, ML 학습 파이프라인 등 배치(Batch) 작업은 태스크 완료 시간(Turnaround Time)과 전체 처리량(Throughput)이 핵심 지표다.
이런 환경에서 RR의 빈번한 컨텍스트 스위칭은 순수한 오버헤드로 작용하며, FCFS나 SJF(Shortest Job First) 대비 전체 처리량이 낮아진다.
태스크 간 우선순위가 명확히 구분되는 시스템(예: 긴급 알림 vs. 일반 리포트 생성)에서는 Priority 스케줄링이 더 적합하다.

| 서비스 유형 | RR 적합 여부 | 이유 |
|------------|:-----------:|------|
| 웹/REST API 서버 | ✅ 적합 | 다수 요청의 공정한 응답 보장, 기아 없음 |
| 인터랙티브 애플리케이션 (채팅, 게임) | ✅ 적합 | 낮고 예측 가능한 응답 지연 |
| 범용 모니터링 데몬 (본 시스템) | ✅ 적합 | 여러 감시 태스크를 균등 주기로 순환 처리 |
| 대용량 배치 처리 서버 | ❌ 부적합 | 컨텍스트 스위칭 오버헤드로 처리량 저하 |
| 우선순위 기반 실시간 제어 시스템 | ❌ 부적합 | 긴급 태스크도 순서를 기다려야 하므로 지연 위험 |
